...
function visitTweet() {
	...
	window.open(
		`https://twitter.com/${props.user.screen_name}/status/${tweetId}`,
		"_blank"
	)
}

const visited = twitterStore.visitedTweets.find(
	(tweet) => tweet.id === props.tweet.id_str
)
...
<template>
	<div class="w-2/12 relative px-2 my-2">
		...
		>
			...
			<span
				class="text-xs absolute top-2 right-2 p-1 bg-zinc-600 rounded-md opacity-40"
			>
				{{ createdAt }}
			</span>
			<div
				v-if="renderMentions"
				class="flex flex-row flex-wrap mt-4 mb-2"
			>
				<template v-for="mention in userMentions" :key="mention.id_str">
					...
					>
						<img
							v-if="
								!!getUser(mention.id_str)
									?.profile_image_url_https
							"
							:src="
								getUser(mention.id_str)?.profile_image_url_https
							"
							alt="mention.name"
			...
			>
				...
				<span
					v-if="isVisited"
					class="text-xs scale-95 inline-block text-zinc-200"
					>({{ visitedDate() }})</span
				>
			</div>
			<div class="my-1" />
