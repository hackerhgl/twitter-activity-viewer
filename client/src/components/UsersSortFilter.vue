...
function getParsedUserData() {
	const mappedUsers = indexes.map((index) => ({
		...index,
		...getUser(index.user)
	}))
	const users = mappedUsers.filter((user) => {
		...
		const nameFilter =
			...
			user.screen_name.toLowerCase().includes(value)
		const nameCheck = twitterStore.userFilterByName.length
			? nameFilter
			: true
		if (!twitterStore.includeNoActionUsers) {
			const visited = twitterStore.visitedTweets
			const everyTweetVisited = user.tweets.every((userTweetId) =>
				visited.find(
					(visitedTweetId) => visitedTweetId.id === userTweetId
				)
			)
	...
	return orderBy(users, (v) => v.tweets.length, [
		twitterStore.reverseUsersByActions ? "asc" : "desc"
	])
}
</script>
...
<template>
	<div id="filters" class="">
		<div
			class="flex flex-row flex-nowrap overflow-auto w-full items-center"
		>
			<h2 class="text-xl">Users filters :</h2>
			<template v-for="filter in userFilters" :key="filter.value">
				<div
					:class="
						getButtonStyle(twitterStore.userFilter === filter.value)
					"
			...
			<div
				...
				@click="
					() =>
						(twitterStore.reverseUsersByActions =
							!twitterStore.reverseUsersByActions)
				"
			>
				Reverse by actions
