...
export const useTwitterStore = defineStore(
	...
	() => {
		...
		const showAfterInitialDate = data.reduce((prev, current) =>
			prev.created_at < current.created_at ? prev : current
		).created_at
		const showBeforeInitialDate = data.reduce((prev, current) =>
			prev.created_at > current.created_at ? prev : current
		).created_at
		const showAfterInitial = dayjs(showAfterInitialDate)
			.subtract(1, "day")
			.toDate()
		const showBeforeInitial = dayjs(showBeforeInitialDate)
			.add(1, "day")
			.toDate()
		...
		function filterUserExist(tweet: Tweet) {
			return users.value.filter((user) => {
				...
				const inReplyCheck =
					user === tweet.in_reply_to_user_id?.toString()
				const entitiesMentionCheck = includeMentionedUsers.value
					? tweet.entities?.user_mentions?.some(
							(mention) => mention.id_str === user
					  )
					: false
				const entitiesMediumCheck = tweet.entities?.media?.some(
					(medium) => medium.source_user_id_str === user
				)
				const extendedEntities = tweet.extended_entities?.media?.some(
					(medium) => medium.source_user_id_str === user
				)
		...
		function checkUserFilter(tweet: Tweet) {
			...
			const checksArray = [included, excluded]
			const checks = checksArray.some((check) => check === true)
			return checks
		}

		function filterDate(tweet: Tweet) {
			const showAfterCheck = dayjs(tweet.created_at).isAfter(
				showAfter.value
			)
			const showBeforeCheck = dayjs(tweet.created_at).isBefore(
				showBefore.value
			)
			const checksArray = [showAfterCheck, showBeforeCheck]
		...
		const filtered = computed(() => {
			const cleaned = data.filter((tweet) => {
				...
				const textCheck = tweet.text
					.toLowerCase()
					.includes(tweetTextSearch.value.toLowerCase())
				const userMentionsFilter = tweet.entities?.user_mentions?.some(
					(mention) => {
						const value =
							tweetUserMentionsSearch.value.toLowerCase()
						return (
							mention.screen_name.toLowerCase().includes(value) ||
							mention.name.toLowerCase().includes(value)
						)
					}
				)
				const userMentionsCheck =
					tweetUserMentionsSearch.value === "" || userMentionsFilter

				const includedVisitedCheck = includeVisitedTweets.value
					? true
					: !visitedTweets.value.some(
							(visited) => visited.id === tweet.id_str
					  )
				const onlyVisitedCheck = onlyVisitedTweets.value
					? visitedTweets.value.some(
							(visited) => visited.id === tweet.id_str
					  )
					: true
				const visitCheck = onlyVisitedTweets.value
					? onlyVisitedCheck
					: includedVisitedCheck

				const includedRetweetCheck = includeRetweets.value
					? true
					: !tweet.retweeted_status
				const onlyRetweetCheck = onlyRetweets.value
					? tweet.retweeted_status
					: true
				const retweetCheck = onlyRetweets.value
					? onlyRetweetCheck
					: includedRetweetCheck

				let filterCountCheck = true
				if (filterTweetsByUserActions.value) {
					const tweetsCount =
						indexes.find((index) => index.user === tweet.user)
							?.tweets?.length ?? 0
					const count = parseInt(
						filterTweetsByUserActionsCount.value,
						10
					)
					const greater = tweetsCount > count
					const lesser = tweetsCount < count
					const check = filterTweetsByUserActionsGreater.value
						? greater
						: lesser
			...
			if (sortVisitedTweets.value) {
				...
				sorted = orderBy(
					cleaned,
					(value) => {
						const visited = visitedTweets.value.find(
							(v) => v.id === value.id_str
						)
						if (visited) {
							const parsedDate = isString(visited.date)
								? dayjs(visited.date).toDate()
								: visited.date
				...
				sorted = orderBy(
					cleaned,
					(tweet) => dayjs(tweet.created_at).toDate().getTime(),
					reverseByDate.value ? "asc" : "desc"
				)
			}
		...
		function addToVisitedTweets(id: string) {
			const index = visitedTweets.value.findIndex(
				(tweet) => tweet.id === id
			)
			if (index > -1) {
		...
		function toggleFilterTweetsByUserActionsGreater() {
			filterTweetsByUserActionsGreater.value =
				!filterTweetsByUserActionsGreater.value
		}
